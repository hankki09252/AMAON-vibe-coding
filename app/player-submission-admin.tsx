...
export default function PlayerSubmissionAdmin() {
  ...
  async function review(item: Submission, action: "approve" | "reject") {
    ...
    const approvalQuestion = transferSchool
      ? `${item.school_name} ${item.player_name} 선수를 ${transferSchool}(으)로 전학 처리할까요? 기존 사진·영상·프로필도 함께 이동합니다.`
      : `${item.school_name} ${item.player_name} 선수의 ${label} 요청을 반영할까요?`;
    if (action === "approve" && !window.confirm(approvalQuestion)) return;
    ...
    setNotice(action === "approve" ? transferSchool ? `${transferSchool} 전학 승인과 선수 자료 이동을 완료했습니다.` : `${label} 승인과 선수 프로필 반영을 완료했습니다.` : "요청을 반려했으며 업로드 파일은 삭제했습니다.");
    await load();
  }
  ...
  return <section className="video-review-panel player-submission-review">
    <div className="member-admin-heading"><div><h3>선수 사진·프로필·전학 승인</h3><p>대표사진, 앨범사진, 프로필 수정과 전학 요청을 확인한 뒤 반영하세요.</p></div><button type="button" onClick={() => void load()}>새로고침</button></div>
