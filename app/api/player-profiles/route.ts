...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = database().prepare(
      "CREATE TABLE IF NOT EXISTS player_profile_overrides (team_id TEXT NOT NULL, player_id TEXT NOT NULL, position TEXT NOT NULL, height INTEGER NOT NULL, weight INTEGER NOT NULL, updated_at INTEGER NOT NULL, updated_by TEXT NOT NULL, PRIMARY KEY (team_id, player_id))",
    ).run().then(() => undefined);
  }
  return schemaReady;
}
...
export async function GET(request: Request) {
  ...
  const result = await database().prepare(
    "SELECT player_id, position, height, weight, updated_at FROM player_profile_overrides WHERE team_id = ?",
  ).bind(teamId).all<{ player_id: string; position: string; height: number; weight: number; updated_at: number }>();
...
export async function PUT(request: Request) {
  ...
  const body = await request.json() as { teamId?: string; playerId?: string; position?: string; height?: number; weight?: number };
  ...
  await database().prepare(
    "INSERT INTO player_profile_overrides (team_id, player_id, position, height, weight, updated_at, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(team_id, player_id) DO UPDATE SET position = excluded.position, height = excluded.height, weight = excluded.weight, updated_at = excluded.updated_at, updated_by = excluded.updated_by",
  ).bind(teamId, playerId, position, height, weight, updatedAt, updatedBy).run();
  return Response.json({ playerId, position, height, weight, updatedAt });
}
