...
export default function VideoSubmissionAdmin() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const response = await fetch("/api/video-submissions", { cache: "no-store" });
    const payload = await response.json().catch(() => ({})) as { items?: Submission[]; error?: string };
    setLoading(false);
    if (!response.ok) return setNotice(payload.error || "영상 등록 요청을 불러오지 못했습니다.");
  ...
  async function review(item: Submission, action: "approve" | "reject") {
    const reason = action === "reject" ? window.prompt("반려 사유를 입력해 주세요.", "영상 내용 또는 선수 관계 확인 필요") : "";
    if (action === "reject" && reason === null) return;
    if (action === "approve" && !window.confirm(`${item.school_name} ${item.player_name} 선수의 ${labels[item.category]} 영상을 공개할까요?`)) return;
    setBusyId(item.id); setNotice("");
    const response = await fetch("/api/video-submissions", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ submissionId: item.id, action, reason }) });
    const payload = await response.json().catch(() => ({})) as { error?: string; profileUrl?: string };
    setBusyId("");
    if (!response.ok) return setNotice(payload.error || "요청을 처리하지 못했습니다.");
    setNotice(action === "approve" ? "영상 승인과 선수 프로필 연결을 완료했습니다." : "영상을 반려하고 비공개 파일을 삭제했습니다.");
    await load();
  }
  ...
  async function copyProfile(item: Submission) {
    ...
    setNotice(`${item.player_name} 선수 프로필 링크를 복사했습니다.`);
  }

  const pending = items.filter((item) => item.status === "pending");
  const history = items.filter((item) => item.status !== "pending").slice(0, 12);
  return <section className="video-review-panel">
    <div className="member-admin-heading"><div><h3>선수 영상 승인</h3><p>승인 전 영상은 비공개입니다. 재생해 확인한 뒤 공개하거나 삭제하세요.</p></div><button type="button" onClick={() => void load()}>새로고침</button></div>
