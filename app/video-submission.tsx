...
type SubmissionMode = "video" | "profile_photo" | "photo" | "profile" | "transfer";
...
const modeLabels: Record<SubmissionMode, { title: string; description: string; icon: string }> = {
  ...
  transfer: { title: "전학 요청", description: "옮긴 학교만 간단히 선택", icon: "⇄" },
};
...
export default function VideoSubmission({ open, players, onClose }: { open: boolean; players: PlayerOption[]; onClose: () => void }) {
  ...
  function chooseMode(nextMode: SubmissionMode) {
    ...
    setProfileForm((current) => ({ ...current, transferTeamId: "" }));
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const requestBody = isVideo
        ...
        : { teamId: selected.sectionId, playerId: selected.player.id, submissionType: mode === "transfer" ? "profile" : mode, relationship, contact: contact.trim(), consent, socialConsent, profileData: mode === "transfer" ? { transferTeamId: profileForm.transferTeamId } : { ...profileForm, transferTeamId: "" }, fileName: file?.name, fileSize: file?.size, contentType, files: mode === "photo" ? photoFiles.map((item) => ({ fileName: item.name, fileSize: item.size, contentType: fileContentType(item, mode) })) : undefined };
