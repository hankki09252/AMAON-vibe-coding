...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 390]) {
      ...
      let mode = "valid", mediaRequests = 0;
      await page.route("**/api/**", async (route) => {
        ...
        if (url.pathname === "/api/media") { mediaRequests++; await pause(1200); }
        if (url.pathname.startsWith("/api/team-")) data = { items: {}, banner: null, emblem: null };
